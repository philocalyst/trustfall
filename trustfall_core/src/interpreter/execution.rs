...
//! does not buffer, collect, or split adapter batches: the internal `SyncAdapter` turns
//! each ready stream into a lazy iterator, and the internal `ReadyIterator` exposes the
//! result stream the same way.
