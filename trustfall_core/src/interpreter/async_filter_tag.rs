//! Async `@filter` with a `@tag` argument, for the [`async_execution`](super::async_execution)
//! engine.
//!
...
use super::{
    ...
    async_execution::{FallibleContextStream, finish_stage, strip_errors},
...
pub(super) fn apply_tag_comparison<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    match tag_ref {
        FieldRef::ContextField(context_field) => {
            apply_context_field_tagged_filter(
                adapter,
                carrier,
                component,
                current_vid,
...
fn apply_tagged_op(filter: &Operation<(), Argument>, left: &FieldValue, right: &FieldValue) -> bool {
    match filter {
