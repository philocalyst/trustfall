...
impl<'a> Adapter<'a> for FaultyAdapter {
    ...
    fn resolve_starting_vertices(
        ...
        let inner =
            self.inner.resolve_starting_vertices(edge_name, parameters, resolve_info).map(unwrap_ok);
        if self.fault == Fault::StartingVertices {
            let remaining = self.remaining.clone();
            Box::new(inner.map(move |v| if should_error(&remaining) { Err(TestError) } else { Ok(v) }))
        } else {
    ...
    fn resolve_neighbors<V: AsVertex<Self::Vertex> + 'a>(
        ...
        Box::new(inner.map(move |(ctx, neighbors)| {
            ...
            let out: VertexIterator<'a, Result<Self::Vertex, Self::Error>> = if faulted {
                ...
                Box::new(
                    neighbors
                        .map(move |v| if should_error(&remaining) { Err(TestError) } else { Ok(v) }),
                )
            } else {
...
fn run(fault: Fault, fail_after: usize, query: &str) -> Vec<Result<Row, ExecutionError<TestError>>> {
