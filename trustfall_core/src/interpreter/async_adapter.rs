...
//!
//! Unlike the sync engine (which keeps its internals infallible and threads errors through a
//! side-channel slot), the async engine threads `Result` **natively** through its streams and
//! fails fast on the first `Err` via `?`. Accordingly, resolver *outputs* carry `Result`s in the
...
///
/// The most ergonomic way to implement an async adapter is usually
/// [`AsyncBasicAdapter`](super::async_basic_adapter::AsyncBasicAdapter), which provides a blanket
