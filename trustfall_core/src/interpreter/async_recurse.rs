//! Async `@recurse` edge expansion for the [`async_execution`](super::async_execution) engine.
//!
...
use super::{
    DataContext,
    ResolveEdgeInfo, ResolveInfo,
    async_adapter::AsyncAdapter,
    async_execution::{FallibleContextStream, finish_stage, finish_stage_plain, strip_errors},
...
pub(super) fn expand_recursive_edge<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    for _ in 2..=max_depth {
        if let Some(coerce_to) = recursive.coerce_to.as_ref() {
            ...
            let (plain, upstream_error) = strip_errors(recursion_stream);
...
fn perform_one_recursive_edge_expansion<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    let query = carrier.query.take().expect("query was not returned");
    let resolve_info =
        ResolveEdgeInfo::new(query, expanding_from.vid, expanding_to.vid, edge_id);
    let edge_outcomes =
        adapter.resolve_neighbors(plain, expanding_from_type, edge_name, edge_parameters, &resolve_info);
    carrier.query = Some(resolve_info.into_inner());
