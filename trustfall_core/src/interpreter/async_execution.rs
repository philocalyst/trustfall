//! Asynchronous, `Stream`-native query interpreter.
//!
//! This is a parallel implementation of the synchronous engine in
//! [`execution`](super::execution), built on [`Stream`]s instead of [`Iterator`]s so that adapter
//! IO can overlap across the contexts in a batch. It is validated by *differential testing*: for
//! every query, the async engine must produce exactly the same results as the (battle-tested)
//! sync engine.
//!
...
//! however, take *plain* context streams and return outcomes carrying `Result`. Each stage bridges
//! the two with [`strip_errors`] (peel the `Ok` contexts for the adapter, stash the first upstream
//! error) and [`finish_stage`] (thread the adapter's `Result` outcomes through `?`, then
//! re-surface the stashed upstream error after the adapter's outputs) — preserving both the
//! adapter's one-outcome-per-context contract and fail-fast semantics.
//!
...
/// A handle to the first upstream error stripped off a [`FallibleContextStream`] before an adapter
/// call, to be re-surfaced after the adapter's outputs. Single-task, so `Rc<RefCell<..>>`.
pub(super) type UpstreamError<E> = Rc<RefCell<Option<E>>>;

/// The async counterpart of [`interpret_ir`](super::execution::interpret_ir).
///
/// Returns a [`Stream`] of results (or the first execution error, fail-fast). Query parsing and
...
> {
    ...
    let starting = adapter.resolve_starting_vertices(&root_edge, &root_edge_parameters, &resolve_info);
...
pub(super) fn strip_errors<'vertex, V, E>(
    input: FallibleContextStream<'vertex, V, E>,
) -> (ContextStream<'vertex, V>, UpstreamError<E>)
where
    V: Clone + Debug + 'vertex,
    E: 'vertex,
{
    let handle: UpstreamError<E> = Rc::new(RefCell::new(None));
    ...
    E: 'vertex,
{
    Box::pin(try_stream! {
        ...
        let pending = upstream_error.borrow_mut().take();
...
/// (tag filters via [`super::async_filter_tag`]).
fn apply_local_field_filter<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    let (plain, upstream_error) = strip_errors(stream);
    ...
    if matches!(filter_without_field.right(), Some(Argument::Tag(_))) {
        return super::async_filter_tag::apply_tagged_filter(
            adapter, carrier, component, current_vid, filter, with_value,
        );
    }
        ...
        if let Some(unary) = unary_filter_predicate(&filter_without_field) {
            ...
            match filter_without_field.right() {
                Some(Argument::Variable(var)) => {
                    let right_value = carrier
                        .query
                        .as_ref()
                        .expect("query was not returned")
                        .arguments[var.variable_name.as_ref()]
                    .clone();
...
fn expand_edge<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    let expanded = if let Some(recursive) = &edge.recursive {
        super::async_recurse::expand_recursive_edge(
            adapter.clone(),
            carrier,
            component,
...
pub(super) fn expand_non_recursive_edge<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    let edge_outcomes =
        adapter.resolve_neighbors(plain, &expanding_from.type_name, edge_name, edge_parameters, &resolve_info);
    carrier.query = Some(resolve_info.into_inner());
...
fn apply_fold_specific_filter<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    if let Some(Argument::Tag(tag_ref)) = filter_without_field.right() {
        ...
        return super::async_filter_tag::apply_tag_comparison(
            adapter,
            carrier,
            parent_component,
            current_vid,
...
async fn compute_fold_outputs<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    if !fold_contains_elements {
        ...
        let mut output_stream: FallibleContextStream<'query, AdapterT::Vertex, AdapterT::Error> =
            Box::pin(futures_util::stream::iter(elements.into_iter().map(Ok::<_, AdapterT::Error>)));
        ...
        for output_name in output_names.iter() {
            ...
            let (plain, upstream_error) = strip_errors(moved);
            let query = carrier.query.take().expect("query was not returned");
...
fn compute_fold<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    for imported_field in fold.imported_tags.iter() {
        match imported_field {
            FieldRef::ContextField(field) => {
                ...
                let (plain, upstream_error) = strip_errors(activated);
                let query = carrier.query.take().expect("query was not returned");
                let resolve_info = ResolveInfo::new(query, vertex_id, true);
                let field_data =
            ...
            FieldRef::FoldSpecificField(fold_specific_field) => {
                ...
                let kind = fold_specific_field.kind.clone();
                stream = Box::pin(stream.map(move |result| {
                    result.map(|mut context| {
    ...
    let activated: FallibleContextStream<'query, AdapterT::Vertex, AdapterT::Error> = Box::pin(
        stream.map(move |result| result.map(|context| context.activate_vertex(&expanding_from_vid))),
    );
    ...
    let edge_outcomes =
        adapter.resolve_neighbors(plain, &type_name, &fold.edge_name, &fold.parameters, &resolve_info);
    carrier.query = Some(resolve_info.into_inner());
    ...
    let min_fold_size = if let Some(min_fold_size) = get_min_fold_count_limit(carrier, fold.as_ref())
    {
    ...
    let folded_stream: FallibleContextStream<'query, AdapterT::Vertex, AdapterT::Error> =
        Box::pin(try_stream! {
...
fn construct_outputs<'query, AdapterT: AsyncAdapter<'query> + 'query>(
    ...
    Box<
        dyn Stream<Item = Result<BTreeMap<Arc<str>, FieldValue>, AdapterT::Error>> + 'query,
    >,
> {
    ...
    let mut output_names: Vec<Arc<str>> =
        root_component.outputs.keys().cloned().collect();
    output_names.sort_unstable(); // deterministic resolve_property() ordering
    ...
    for output_name in output_names.iter() {
        ...
        let (plain, upstream_error) = strip_errors(moved_stream);
