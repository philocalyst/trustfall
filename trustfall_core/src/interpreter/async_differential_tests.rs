//! Differential tests: the async engine must produce byte-for-byte the same results as the
//! (battle-tested) sync engine for numbers-schema queries, including `@filter` / `@fold` /
//! `@recurse` (which the async engine implements).
//!
...
use crate::{
    frontend::parse,
    interpreter::{
        async_helpers::SyncToAsyncAdapter, execution::interpret_ir,
    },
    ir::FieldValue,
    numbers_interpreter::NumbersAdapter,
};

use super::async_execution::interpret_ir_async;
...
/// Assert the async engine matches the sync engine on `query`, and that the result is non-empty
...
#[test]
fn nested_edge() {
    assert_matches_sync(r#"{ Number(min: 0, max: 6) { value @output successor { s: value @output } } }"#);
}
...
/// Helper: run the sync engine for a numbers query with the given arguments.
fn sync_results_with_args(
    query: &str,
...
/// Run the async engine; any panic during construction *or* collection is returned as `Err`.
...
fn corpus_differential_sweep() {
    ...
    let base = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("test_data/tests/valid_queries");
    ...
    for entry in &entries {
        ...
        assert_eq!(
            test_query.schema_name, "numbers",
    ...
    if !divergences.is_empty() {
        msg.push_str(&format!(
            "\n\nFAILURE: {} query/queries diverged between async and sync engines!\n\n",
            divergences.len()
        ));
    ...
    if !panics.is_empty() {
        msg.push_str(&format!(
            "\n\nFAILURE: {} query/queries panicked in the async engine!\n\n",
            panics.len()
        ));
...
fn assert_empty_matches_sync(query: &str) {
    ...
    let stream = interpret_ir_async(adapter2, indexed2, Arc::new(BTreeMap::new()))
        .expect("unexpected arguments error");
    let actual: Vec<Row> =
        futures_executor::block_on(async { stream.map(|r| r.expect("infallible")).collect().await });
