...
use crate::{
    ...
    interpreter::{
        ...
        async_execution::interpret_ir_async,
        async_helpers::{
            SyncToAsyncAdapter, map_contexts_buffered, try_resolve_property_with_concurrent,
        },
...
impl<'a> AsyncAdapter<'a> for FaultyAsyncAdapter {
    ...
    fn resolve_neighbors<V: AsVertex<Self::Vertex> + 'a>(
        ...
        Box::pin(outcomes.map(move |(ctx, neighbors)| {
            ...
            let out: VertexStream<'a, Result<Self::Vertex, Self::Error>> = if faulted {
...
fn sync_to_async_streams_one_context_at_a_time() {
    ...
    let contexts: ContextStream<'_, u64> = Box::pin(stream::iter(
        (0u64..5).map(|v| DataContext::new(Some(v))),
    ));
...
fn map_contexts_buffered_preserves_order_under_concurrency() {
    let contexts: ContextStream<'_, u64> = Box::pin(stream::iter(
        (0u64..20).map(|v| DataContext::new(Some(v))),
    ));
    ...
    let values: Vec<u64> = futures_executor::block_on(async {
        out.map(|(_ctx, id)| id).collect().await
    });
...
fn try_resolve_property_concurrent_preserves_order_and_nulls() {
    ...
    assert_eq!(
        values,
        vec![
            Ok(FieldValue::Int64(10)),
            Ok(FieldValue::Null),
            Ok(FieldValue::Int64(30)),
        ]
    );
}
...
fn contract_too_few_property_outcomes_truncates_results() {
    ...
    use super::async_execution::{FallibleContextStream, finish_stage, strip_errors};

    let numbers = NumbersAdapter::new();
    let schema = numbers.schema().clone();
    let input: FallibleContextStream<'_, u64, TestError> =
        Box::pin(stream::iter((0u64..5).map(|v| Ok(DataContext::new(Some(v))))));
    let (plain, upstream) = strip_errors(input);
